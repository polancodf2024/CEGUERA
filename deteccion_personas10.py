...
class SistemaDeteccionPersonasStreamlit:
    ...
    def detectar_entorno_cloud(self):
        ...
        # Verificar si estamos en un entorno sin display (headless)
        if os.getenv('DISPLAY') is None and os.name != 'nt':
            print("🔍 Entorno sin display detectado (posible Cloud)")
            return True
    ...
    def inicializar_audio_mejorado(self):
        """Configura audio mejorado y más confiable - SOLO EN LOCAL"""
        if self.modo_cloud:
            print("🔇 Audio desactivado para Streamlit Cloud")
            return
        ...
        try:
            ...
            # Inicializar pygame en modo headless para audio
            pygame.init()
            pygame.mixer.init(
                frequency=22050,
                size=-16,
            ...
            # Verificar si el mixer se inicializó correctamente
    ...
    def reproducir_sonido(self, zona):
        """Reproduce sonido en un hilo separado para no bloquear"""
    ...
    def probar_audio(self):
        ...
        if not self.audio_disponible:
            return "🔇 Audio no disponible en Streamlit Cloud"
        ...
        try:
            ...
            for zona in ['izquierda', 'frente', 'derecha', 'alerta']:
                if zona in self.sonidos:
                    ...
                    mensajes.append(f"🔊 {zona}")
                    time.sleep(0.9)
            
            return "✅ Sonidos probados: " + ", ".join(mensajes)
        except Exception as e:
    ...
    # ===== FUNCIONES DE DETECCIÓN (se mantienen igual) =====
    ...
    # ===== FUNCIONALIDAD DE CÁMARA (solo local) =====
    def iniciar_camara(self):
        if self.modo_cloud:
            return False, "❌ La cámara no está disponible en Streamlit Cloud"
        try:
    ...
    # ===== FUNCIONALIDAD DE ARCHIVOS (para Cloud) =====
...
def main():
    ...
    if sistema.modo_cloud:
        st.title("🎯 Sistema de Detección - STREAMLIT CLOUD")
        st.info("☁️ **Modo Cloud**: Sube archivos para procesar (sin cámara/audio)")
    else:
        st.title("🎯 Sistema de Detección - MODO LOCAL")
        st.success("💻 **Modo Local**: Cámara y audio disponibles")
    ...
    with st.sidebar:
        ...
        # Selección de modo (solo en local)
        ...
        if modo_actual == "cámara" and not sistema.modo_cloud:
        ...
        # Controles de audio (solo en local)
        if not sistema.modo_cloud:
            st.markdown("---")
            st.subheader("🔊 Audio")
        ...
        st.markdown("---")
        st.subheader("📊 Estado")
        st.info(f"🔍 Detectores: {len(sistema.detectores)}")
        if not sistema.modo_cloud:
            st.info(f"🔊 Audio: {'✅' if sistema.audio_disponible else '❌'}")
            st.info(f"📷 Cámara: {'✅' if st.session_state.camara_activa else '❌'}")
        st.info(f"🎯 Factor: {sistema.calibracion_distancia['factor_ajuste_camara']:.3f}")
    ...
    if sistema.modo_cloud:
        ...
        st.markdown("**Sistema de Detección** | 💻 **Modo Local** | 🎥 **Cámara + 🔊 Audio**")
